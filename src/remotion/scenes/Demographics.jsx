      ...
      {segments.map((seg, i) => {
        ...
        const el = (
          <circle
            ...
            strokeLinecap="round"
            opacity={interpolate(frame, [startFrame + i * 4, startFrame + i * 4 + 8], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' })}
          />
        );
...
function HBar({ label, value, maxValue, color, frame, startFrame, suffix = '%' }) {
  const barProgress = interpolate(frame, [startFrame, startFrame + 20], [0, 1], {
    extrapolateLeft: 'clamp', extrapolateRight: 'clamp',
  });
...
function StatCard({ icon, value, label, color, frame, startFrame }) {
  ...
  return (
    <div style={{
      ...
      <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.45)', margin: 0, fontWeight: 600 }}>{label}</p>
    </div>
  );
}
...
function PlatformBar({ platform, pcts, frame, startFrame }) {
  ...
  return (
    <div style={{ opacity: op, display: 'flex', alignItems: 'center', gap: 8, marginBottom: 5 }}>
      <span style={{ fontSize: 13, fontWeight: 700, color: colors[platform] || '#fff', width: 28, textAlign: 'right' }}>{platform}</span>
      <div style={{ flex: 1, display: 'flex', gap: 2, height: 14 }}>
        {pcts.map((p, i) => {
          const barW = interpolate(frame, [startFrame + i * 3, startFrame + i * 3 + 12], [0, p.value], {
            extrapolateLeft: 'clamp', extrapolateRight: 'clamp',
          });
          return (
...
export const Demographics = () => {
  ...
  const userTypes = [
    ...
    { label: 'Istri (KBT)', pct: 0.22, color: '#0F766E', count: '350K', icon: '👩', desc: 'Perempuan, usia 20-50' },
    { label: 'Anak (KBT)', pct: 0.50, color: '#6D28D9', count: '800K', icon: '👦', desc: 'Usia 13-25, digital native' },
    { label: 'Suami (KBT)', pct: 0.03, color: GOLD, count: '50K', icon: '👨', desc: 'Laki-laki, usia 22-55' },
  ];
    ...
    <AbsoluteFill style={{ background: DARK, fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ position: 'absolute', top: 105, bottom: 30, left: 40, right: 40, display: 'flex', gap: 24 }}>
        ...
        <div style={{ flex: '0 0 380px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '12px 16px', borderRadius: 14, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)' }}>
            <div style={{ position: 'relative' }}>
              <DonutChart segments={userTypes} size={140} strokeWidth={16} frame={frame} startFrame={35} />
              <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
                <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.3)', fontWeight: 600 }}>TIPE</span>
                <span style={{ fontSize: 16, fontWeight: 900, color: '#fff' }}>AKUN</span>
              </div>
            </div>
            <div style={{ flex: 1 }}>
              {userTypes.map((t, i) => {
                const op = interpolate(frame, [40 + i * 5, 48 + i * 5], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
                return (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, opacity: op }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <span style={{ fontSize: 13, fontWeight: 800, color: t.color, fontFamily: "'JetBrains Mono'" }}>{t.count}</span>
                      </div>
          ...
          <div style={{ padding: '12px 16px', borderRadius: 14, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            {platformByDemo.map((p, i) => (
              <PlatformBar key={i} platform={p.platform} pcts={p.pcts} frame={frame} startFrame={90 + i * 6} />
            ))}
          </div>
        </div>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {/* Key insight */}
          {(() => {
            const insightOp = interpolate(frame, [125, 145], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
            return (
              <div style={{ padding: '14px 18px', borderRadius: 14, background: 'rgba(184,134,11,0.06)', border: '1px solid rgba(184,134,11,0.15)', opacity: insightOp }}>
                ...
                <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)', margin: 0, lineHeight: 1.5 }}>
                  Kombinasi prajurit aktif (konten otoritatif) + KBT muda (viral power) + anak digital native (engagement tinggi) = ekosistem konten yang lengkap dan organik
                </p>
              </div>
            );
          })()}
        ...
        <div style={{ flex: '0 0 340px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '12px 16px', borderRadius: 14, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)' }}>
            <div style={{ position: 'relative' }}>
              <DonutChart
                ...
                size={130} strokeWidth={14} frame={frame} startFrame={70}
              />
              <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
                <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.3)', fontWeight: 600 }}>15</span>
            ...
            <div style={{ flex: 1 }}>
              ...
              {regions.map((r, i) => {
                const op = interpolate(frame, [75 + i * 4, 83 + i * 4], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
                return (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3, opacity: op }}>
                    ...
                    <span style={{ fontSize: 12, fontWeight: 700, color: r.color, fontFamily: "'JetBrains Mono'" }}>{r.value}%</span>
                  </div>
                );
              })}
            </div>
          ...
          <div style={{ padding: '14px 18px', borderRadius: 14, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            {[
              ...
              <HBar key={i} label={e.label} value={e.value} maxValue={40} color={e.color} frame={frame} startFrame={100 + i * 6} />
            ))}
          </div>
          ...
          <div style={{ padding: '14px 18px', borderRadius: 14, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {[
                ...
                const op = interpolate(frame, [115 + i * 8, 125 + i * 8], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
                return (
                  <div key={i} style={{ flex: 1, padding: '8px 10px', borderRadius: 8, background: 'rgba(255,255,255,0.03)', textAlign: 'center', opacity: op }}>
                    ...
                    <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', margin: '2px 0 0' }}>{d.label}</p>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
              {[
                ...
                const op = interpolate(frame, [130 + i * 5, 140 + i * 5], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
                return (
                  <div key={i} style={{ flex: 1, textAlign: 'center', opacity: op }}>
                    ...
                    <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)', margin: '2px 0 0' }}>{s.label}</p>
                  </div>
                );
              })}
            </div>
