...
function AdminDashboardMini({ opacity }) {
  return (
    <div style={{ opacity, width: 280, height: 160, borderRadius: 10, background: '#0B2619', border: '1px solid rgba(255,255,255,0.1)', overflow: 'hidden', boxShadow: '0 6px 24px rgba(0,0,0,0.4)' }}>
      <div style={{ padding: '5px 8px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', gap: 4 }}>
        ...
        <span style={{ fontSize: 7, color: 'rgba(255,255,255,0.3)', flex: 1, textAlign: 'center' }}>SINAR Command Center</span>
      </div>
      <div style={{ padding: '8px' }}>
        <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 6, padding: '6px 8px', marginBottom: 6 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 4 }}>
            <span style={{ fontSize: 7, fontWeight: 700, color: GOLD, background: 'rgba(184,134,11,0.15)', padding: '1px 6px', borderRadius: 3 }}>EVENT</span>
            <span style={{ fontSize: 7, color: 'rgba(255,255,255,0.3)' }}>Prioritas Tinggi</span>
          </div>
          <p style={{ fontSize: 8, fontWeight: 700, color: '#fff', margin: 0 }}>Upacara HUT TNI AD ke-81</p>
          <p style={{ fontSize: 7, color: 'rgba(255,255,255,0.4)', margin: '2px 0 0' }}>Target: 400.000 prajurit</p>
        </div>
        <div style={{ background: '#14532D', borderRadius: 6, padding: '6px', textAlign: 'center' }}>
          <span style={{ fontSize: 8, fontWeight: 800, color: '#4ADE80' }}>KIRIM KE 400K PRAJURIT →</span>
        </div>
      </div>
    </div>
  );
...
function PhoneNotifMini({ opacity }) {
  return (
    <div style={{ opacity, width: 140, height: 220, borderRadius: 16, background: '#111', border: '2px solid rgba(255,255,255,0.1)', overflow: 'hidden', boxShadow: '0 6px 24px rgba(0,0,0,0.4)', position: 'relative' }}>
      <div style={{ padding: '4px 8px 2px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontSize: 7, fontWeight: 600, color: '#fff' }}>09:41</span>
      ...
      <div style={{ padding: '6px 8px' }}>
        ...
        <div style={{ background: 'rgba(74,222,128,0.1)', border: '1px solid rgba(74,222,128,0.2)', borderRadius: 8, padding: '6px 8px', marginTop: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 3 }}>
            ...
            <span style={{ fontSize: 7, fontWeight: 700, color: '#4ADE80' }}>Misi Baru!</span>
          </div>
          <p style={{ fontSize: 7, color: '#fff', margin: 0, lineHeight: 1.3 }}>Upacara HUT TNI AD ke-81</p>
          <p style={{ fontSize: 6, color: 'rgba(255,255,255,0.4)', margin: '2px 0 0' }}>+400 XP · EVENT</p>
        </div>
...
function UploadMini({ opacity }) {
  return (
    <div style={{ opacity, width: 140, height: 220, borderRadius: 16, background: '#111', border: '2px solid rgba(255,255,255,0.1)', overflow: 'hidden', boxShadow: '0 6px 24px rgba(0,0,0,0.4)' }}>
      <div style={{ padding: '4px 8px 2px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontSize: 7, fontWeight: 600, color: '#fff' }}>09:45</span>
        <div style={{ width: 40, height: 8, background: '#fff', borderRadius: 4 }} />
        <span style={{ fontSize: 7, color: '#fff' }}>📶</span>
      </div>
      <div style={{ padding: '6px 8px' }}>
        <p style={{ fontSize: 8, fontWeight: 700, color: '#fff', margin: '4px 0 6px' }}>Upload Bukti</p>
        {/* Photo placeholder */}
        ...
        <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 6, padding: '4px 6px', marginBottom: 6 }}>
          <p style={{ fontSize: 6, color: 'rgba(255,255,255,0.3)', margin: 0 }}>Dokumentasi upacara...</p>
        </div>
        {/* Platform tags */}
        <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
          {['IG', 'YT', 'TT'].map(p => (
            <span key={p} style={{ fontSize: 6, padding: '2px 5px', borderRadius: 3, background: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.4)', fontWeight: 700 }}>{p}</span>
          ))}
        </div>
        <div style={{ background: '#0F766E', borderRadius: 6, padding: '5px', textAlign: 'center', marginTop: 6 }}>
          <span style={{ fontSize: 7, fontWeight: 800, color: '#fff' }}>SUBMIT →</span>
        </div>
      </div>
    </div>
  );
...
function AiCheckMini({ opacity, frame, startFrame }) {
  const checkProgress = interpolate(frame, [startFrame + 20, startFrame + 60], [0, 1], {
    extrapolateLeft: 'clamp', extrapolateRight: 'clamp',
  });
  const checks = ['Format sesuai', 'Hashtag ada', 'Durasi OK', 'Original'];
  return (
    <div style={{ opacity, width: 200, height: 160, borderRadius: 10, background: 'rgba(109,40,217,0.08)', border: '1px solid rgba(109,40,217,0.2)', padding: '12px', boxShadow: '0 6px 24px rgba(0,0,0,0.4)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
        <span style={{ fontSize: 16 }}>🤖</span>
        <span style={{ fontSize: 10, fontWeight: 800, color: '#A78BFA' }}>AI Verification</span>
      </div>
      {checks.map((c, i) => {
        ...
        return (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
            ...
            <span style={{ fontSize: 9, color: done ? 'rgba(255,255,255,0.7)' : 'rgba(255,255,255,0.3)' }}>{c}</span>
          </div>
        );
      })}
      <div style={{ marginTop: 6, padding: '4px 8px', borderRadius: 6, background: checkProgress >= 1 ? 'rgba(74,222,128,0.15)' : 'rgba(255,255,255,0.03)', textAlign: 'center' }}>
        <span style={{ fontSize: 9, fontWeight: 700, color: checkProgress >= 1 ? '#4ADE80' : 'rgba(255,255,255,0.2)' }}>
...
function RewardMini({ opacity }) {
  return (
    <div style={{ opacity, width: 200, height: 160, borderRadius: 10, background: 'rgba(184,134,11,0.06)', border: '1px solid rgba(184,134,11,0.2)', padding: '12px', boxShadow: '0 6px 24px rgba(0,0,0,0.4)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
        <span style={{ fontSize: 16 }}>🏆</span>
        <span style={{ fontSize: 10, fontWeight: 800, color: GOLD }}>Reward Diterima!</span>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, padding: '6px 8px', background: 'rgba(184,134,11,0.1)', borderRadius: 6 }}>
        ...
        <span style={{ fontSize: 9, color: 'rgba(255,255,255,0.5)' }}>XP Earned</span>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
        ...
        <span style={{ fontSize: 7, color: 'rgba(255,255,255,0.4)' }}>Sersan</span>
      </div>
    </div>
  );
}
...
export const OperationalFlow = () => {
  ...
  const titleY = interpolate(frame, [0, 50], [25, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });

  const steps = [
    { label: 'LANGKAH 1', title: 'DISPENAD Buat Misi', sub: 'Admin membuat & broadcast instruksi', color: '#8B1A1A', startFrame: 40 },
    { label: 'LANGKAH 2', title: 'Prajurit Terima', sub: '400K menerima notifikasi di app', color: '#14532D', startFrame: 75 },
    { label: 'LANGKAH 3', title: 'Eksekusi & Upload', sub: 'Hadir, dokumentasi, upload bukti', color: '#0F766E', startFrame: 110 },
    { label: 'LANGKAH 4', title: 'Verifikasi AI', sub: 'Sistem AI cek kualitas otomatis', color: '#6D28D9', startFrame: 145 },
    { label: 'LANGKAH 5', title: 'Reward & XP', sub: 'Poin masuk, pangkat naik', color: GOLD, startFrame: 180 },
  ];
  ...
  return (
    <AbsoluteFill style={{ background: DARK, fontFamily: "'Inter', sans-serif" }}>
      ...
      {/* Main content — step labels on top, mockups below */}
      <div style={{ position: 'absolute', top: 150, left: 0, right: 0, display: 'flex', justifyContent: 'center', alignItems: 'flex-start', gap: 0, padding: '0 40px' }}>

        {steps.map((step, i) => {
          const appear = spring({ frame: frame - step.startFrame, fps, config: { damping: 14, mass: 0.6 } });
          return (
            <React.Fragment key={i}>
              {i > 0 && <Arrow frame={frame} startFrame={step.startFrame - 10} />}
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', opacity: appear, transform: `translateY(${(1 - appear) * 30}px)` }}>
                {/* Step badge */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                  <div style={{ width: 28, height: 28, borderRadius: '50%', background: step.color, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: `0 0 16px ${step.color}40` }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)', margin: 0 }}>{step.sub}</p>
                  </div>
                </div>
                ...
                <div style={{ marginTop: 4 }}>
                  ...
                  {i === 3 && <AiCheckMini opacity={appear} frame={frame} startFrame={step.startFrame} />}
                  {i === 4 && <RewardMini opacity={appear} />}
                </div>
              </div>
            </React.Fragment>
          );
      ...
      {/* Bottom summary */}
      {(() => {
        const sumOp = interpolate(frame, [220, 260], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
        return (
